...
const codeUnitsApi = api.injectEndpoints({
  endpoints: (builder) => ({
    getCodeUnits: builder.query<CodeUnit[], { workspace_id: string; run_id?: number; kind?: string; q?: string; limit?: number; offset?: number }>({
      query: (params) => `/code-units${qs(params)}`,
      transformResponse: (response: { items: CodeUnit[] }) => response.items,
      providesTags: ['CodeUnit'],
    }),
    getCodeUnit: builder.query<CodeUnitDetail, { hash: string; workspace_id: string }>({
