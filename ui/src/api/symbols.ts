...
const symbolsApi = api.injectEndpoints({
  endpoints: (builder) => ({
    getSymbols: builder.query<Symbol[], { workspace_id: string; run_id?: number; kind?: string; exported?: boolean; q?: string; limit?: number; offset?: number }>({
      query: (params) => `/symbols${qs(params)}`,
      transformResponse: (response: { items: Symbol[] }) => response.items,
      providesTags: ['Symbol'],
    }),
    getSymbol: builder.query<Symbol, { hash: string; workspace_id: string }>({
